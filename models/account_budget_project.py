...
class CrossoveredBudgetProject(models.Model):
    ...
    other_currency = fields.Boolean('Cotización en USD', required=True, default=False)
...
class CrossoveredBudgetLinesProject(models.Model):
    ...
    practical_amount2 = fields.Float(
        compute='_compute_percentage2', string='Logro', digits=0)
    company_id = fields.Many2one(related='crossovered_budget_id.company_id', comodel_name='res.company',
                                 string='Company', store=True, readonly=True)
    currency_line = fields.Many2one('res.currency', 'Otra Moneda')
    other_currency_project_enabled = fields.Boolean(compute='_compute_currency')
    multiline_test=fields.Text()

    @api.one
    def _compute_currency(self):            
        for line in self:
             line.other_currency_project_enabled=line.crossovered_budget_id.other_currency        
